...
TreeNode.prototype.appendNode = function (node) {
    var indexOfFirstHidden = this.indexOfFirstHidden();
...
TreeNode.prototype.prepareNodeEditor = function (root) {
    ...
    header.classList.add("editor-node");
    ...
    addNode.classList.add("glyphicon");
    addNode.classList.add("glyphicon-plus");
    addNode.classList.add("btn");
    addNode.classList.add("btn-default");
...
TreeNode.prototype.btnAddNodeClick = function (e) {
    var node = new TreeNode(this.node, this.node.editor, false);
...
TreeNode.prototype.findNode = function (id) {
    ...
    for (var i = 0; i < this.nodes.length; i++) {
        var found = this.nodes[i].findNode(id);
        if (found) {
            return found;
        }
    }
...
TreeEditor.prototype.loadJson = function (rawData) {
    ...
    for (var i = 0; i < obj.length; i++) {
        var parent = this.root.findNode(obj[i].parent_category_id);
        var node = new TreeNode(parent, this, false);
